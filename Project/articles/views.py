...
from Project.articles.models import Article, Tag, ArticleComment,Ingredient
...
def ingredient(request, ingredient_name):
    ingredients = Ingredient.objects.filter(ingredient=ingredient_name)
    articles = []
    for ingredient in ingredients:
        if ingredient.article.status == Article.PUBLISHED:
            articles.append(ingredient.article)
...
def write(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            article.create_ingredients(ingredients)
